...
export async function getBookListFromDatabase(user: User): Promise<BookObject[]> {
    const usr : User | null = await getQuery("USERNAME", "username", user.username)
...
export async function addEntryToDatabase(entry : Entry, book : BookObject, username: string) {
    ...
    await fetch(`http://localhost:3232/database?command=UPDATE&type=BOOK&bookID=${book.bookID}&entryID=${entry.entryID}&username=${username}`);
}
...
export async function getEntryOffID(entryID : string) {
    let response = await fetch(`http://localhost:3232/database?command=QUERY&type=ENTRY&entryID=${entryID}`);
    const json = await response.json();
    console.log(json.result)
    if (json.result == "success") {
        console.log(json)
